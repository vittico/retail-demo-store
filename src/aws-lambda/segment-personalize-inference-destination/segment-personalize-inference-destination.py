...
if (
    'segment_personas_write_key' not in os.environ
    or os.environ['segment_personas_write_key'] == ''
):
...
if (
    'recommendations_service_url' not in os.environ
    or os.environ['recommendations_service_url'] == ''
):
...
def lambda_handler(event, context):
    logger.debug(f"Segment event: {json.dumps(event)}")
